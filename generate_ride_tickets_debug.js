...
async function generateRideTickets() {
  try {
    ...
    console.log(`读取完成: ${trainSchedules.length} 个车次, ${trainScheduleDetails.length} 个站点明细, ${logicStations.length} 个逻辑车站`);
    ...
    // 4.1.2 生成出勤便乘票 - 根据新算法逻辑
    for (let i = 0; i < sortedTrainSchedulesAsc.length; i++) {
      const schedule = sortedTrainSchedulesAsc[i];
      const trainName = schedule.train_name;
      const trainScheduleId = schedule.train_schedule_id;
      
      console.log(`\n处理车次 ${i+1}/${sortedTrainSchedulesAsc.length} ${trainName}`);
      ...
      console.log(`  车次站点数: ${scheduleDetails.length}`);
      
      // 如果没有站点明细，跳过该车次
      if (scheduleDetails.length === 0) {
        console.log('  无站点明细，跳过');
        continue;
      }
      ...
      for (let j = 0; j < scheduleDetails.length; j++) {
        const detail = scheduleDetails[j];
        ...
        if (isAttendanceStation) {
          ...
          firstAttendanceIndex = j;
          console.log(`  找到出勤站点: ${stationMap[detail.station_id]} (索引 ${j})`);
          break;
        }
      }
    ...
    // 4.1.4 生成退勤便乘票 - 根据新算法逻辑
    for (let i = 0; i < sortedTrainSchedulesDesc.length; i++) {
      const schedule = sortedTrainSchedulesDesc[i];
      const trainName = schedule.train_name;
      const trainScheduleId = schedule.train_schedule_id;
      
      console.log(`\n处理退勤车次 ${i+1}/${sortedTrainSchedulesDesc.length} ${trainName}`);
      ...
      for (let j = 0; j < scheduleDetails.length; j++) {
        ...
        if (isOffDutyStation) {
          ...
          firstOffDutyIndex = j;
          console.log(`  找到退勤站点: ${stationMap[detail.station_id]} (索引 ${j})`);
          break;
        }
      }
    ...
    if (rideTickets.length === 0) {
      console.log('注意: 未生成任何便乘票');
    }

...
async function generateRideTickets() {
  try {
    ...
    for (const schedule of sortedTrainSchedulesAsc) {
      ...
      if (firstAttendanceStation) {
        ...
        for (let j = firstAttendanceIndex + 1; j < scheduleDetails.length; j++) {
          ...
          console.log(`    检查站点 ${j}: ${stationMap[nextDetail.station_id]} - 接车站点: ${isPickupStation}, 末站: ${isLastStation}`);
          
          // 如果是接车站点或末站
          if (isPickupStation || isLastStation) {
            const endStationName = stationMap[nextDetail.station_id];
            console.log(`    找到目标站点: ${endStationName} (索引 ${j})`);
        ...
        // 如果没找到接车站点或末站，但出勤站点本身就是末站的情况（理论上不应该发生，但为了完整性）
        if (!foundTicket) {
          console.log(`    未找到合适的下车站点`);
        }
      }
    }
    ...
    for (const schedule of sortedTrainSchedulesDesc) {
      ...
      if (firstOffDutyStation) {
        ...
        for (let j = firstOffDutyIndex + 1; j < scheduleDetails.length; j++) {
          ...
          console.log(`    检查站点 ${j}: ${stationMap[nextDetail.station_id]} - 退车站点: ${isDropoffStation}, 首站: ${isFirstStation}`);
          
          // 如果是退车站点或首站
          if (isDropoffStation || isFirstStation) {
            const startStationName = stationMap[nextDetail.station_id];
            console.log(`    找到目标站点: ${startStationName} (索引 ${j})`);
    ...
    if (finalRideTickets.length > 0) {
      ...
      if (finalRideTickets.length > 10) {
        console.log('... 还有', finalRideTickets.length - 10, '条记录');
      }
    }

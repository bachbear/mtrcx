...
async function debugGenerateRideTickets() {
  try {
    // 计算项目根目录路径
    ...
    console.log('列车时刻明细数量:', trainScheduleDetails.length);
    console.log('逻辑车站数量:', logicStations.length);
    console.log('车站数量:', stations.length);
    ...
    Object.keys(stationMap).forEach(id => {
      console.log(`  ${id}: ${stationMap[id]}`);
    });
    
    console.log('\n=== 逻辑车站详情 ===');
    logicStations.forEach(ls => {
      console.log(`  ID:${ls.id} 站点ID:${ls.station_id} 站点名:${stationMap[ls.station_id]} 类型:${ls.station_type}`);
    });
    ...
    console.log('\n=== 排序后的车次 ===');
    sortedTrainSchedulesAsc.forEach(schedule => {
      console.log(`  ${schedule.train_name} - ${schedule.begin_time} 到 ${schedule.end_time}`);
    });
...
module.exports = { debugGenerateRideTickets };